...
def img_downsize(img, ds):
    dst = cv2.resize(img, dsize=(0, 0), fx=ds, fy=ds,
...
def zoom_img(img, scale):
    ...
    temp_input = cv2.resize(label, dsize=(0, 0), fx=1 /
                            scale, fy=1/scale, interpolation=cv2.INTER_AREA)
    input = cv2.resize(temp_input, dsize=(0, 0), fx=scale,
            ...
            for degree in [0., 1., 2., 3.]:
                ...
                for ds in [1., 0.7, 0.4]:
                    ...
                    for scale in [2, 3, 4]:
                        ...
                        sub_ipt, sub_lab = sub_img(
                            input, label, i_size, l_size, stride)
                        sub_ip += sub_ipt
                        sub_la += sub_lab
        print('data no.', num)
        num += 1
